...
def report_func(epoch, batch, num_batches,
    ...
    if batch % opt.report_every == -1 % opt.report_every:
        ...
        report_stats = train_utils.PairStatistics()
...
#                            opt.normalization, opt.accum_count)
    trainer=train_utils.PairTrainer(model, train_loss, valid_loss, 
                                    optim,model_opt.template_weight,
                                    trunc_size, shard_size, data_type,
                                    opt.normalization, opt.accum_count)
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        print('Train rouge loss: {}'.format(train_stats.rouge_loss()))
        ...
        print('Validation rouge loss: {}'.format(valid_stats.rouge_loss()))
...
def load_fields(dataset, data_type, checkpoint):
    fields=txt_utils.load_fields_from_vocab(torch.load(opt.data + '.vocab.pt'))
...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    model=model_utils.make_base_model(model_opt, fields,use_gpu(opt), checkpoint)
    if len(opt.gpuid) > 1:
